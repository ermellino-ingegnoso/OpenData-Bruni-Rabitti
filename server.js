...
app.get("/appalti/CIG/:cig", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) => appalto["CIG"] === cig.toUpperCase()
    );
...
app.get("/appalti/dataAdOggi/:data", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) => new Date(appalto["DATA PUBBLICAZIONE"]) >= data
    );
    res.json(filteredAppalti);
});
...
app.get("/appalti/data/:data", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) => appalto["DATA PUBBLICAZIONE"] === data
    );
    res.json(filteredAppalti);
});
...
app.get("/appalti/valBaseAsta/:minmax", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) =>
            appalto["VALORE A BASE DI ASTA"] >= min &&
            appalto["VALORE A BASE DI ASTA"] <= max
    );
    res.json(filteredAppalti);
});
...
app.get("/appalti/loc/:loc", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) =>
            appalto["LOCALIZZAZIONE"].toLowerCase() === loc.toLowerCase()
    );
    res.json(filteredAppalti);
});
...
app.get("/appalti/contratto/:contratto", function (req, res) {
    ...
    const filteredAppalti = appalti.filter(
        (appalto) =>
            appalto["TIPOLOGIA CONTRATTO"].toLowerCase() ===
            contratto.toLowerCase()
    );
    res.json(filteredAppalti);
});

app.post("/aggiuntaAppalto", async function (req, res) {
    let obj_esito = Object.fromEntries(
        Object.keys(req.body).map((chiave) => [chiave, true])
    );
    Object.keys(obj_esito).forEach((el) => {
        if (el == "CIG") {
            obj_esito[el] =
                appaltoRegex(req.body[el]) &&
                !controlloDuplicati("CIG", req.body[el]);
        } else if (el == "DATA PUBBLICAZIONE") {
            obj_esito[el] = dataRegex(req.body[el]);
        } else if (
            el == "VALORE A BASE D'ASTA" ||
            el == "VALORE AGGIUDICAZIONE"
        ) {
            obj_esito[el] = valoreRegex(req.body[el]);
        } else if (
            el == "LOCALIZZAZIONE" ||
            el == "TIPOLOGIA CONTRATTO" ||
            el == "AMMINISTRAZIONE APPALTANTE" ||
            el == "OGGETTO"
        ) {
            obj_esito[el] = testoRegex(req.body[el]);
        }
    });
    let arrErrori = Object.keys(obj_esito).filter(
    ...
    if (arrErrori.length == 0) {
        ...
        try {
            ...
            res.status(400).send(
                "Errore nella sovrascrittura del file, appalto non aggiunto"
            );
        }
    } else {
        let testo_errori = arrErrori.reduce(
            (tot, v) => (tot += v + "<br>"),
            ""
        );
